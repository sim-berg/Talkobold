...
Talkobold starts"** setting (on by default — recommended if the hotkey doesn't
fire on your system).
...
2. Under **Virtual microphone** you should see *"running — pick Talkobold Mic
...
```bash
wpctl status | grep -i talkobold     # Talkobold-sink + talkobold-mic appear
```

In Discord/Zoom, select **"Talkobold Mic"** as your microphone; click **Speak**
and your friend hears it, while the monitor path lets you hear yourself.

## Building the Windows installer (one-click `.exe`)

Talkobold packages to a **one-click NSIS installer** via `electron-builder`.
The installer needs no admin, installs per-user, adds Start Menu + desktop
shortcuts, and launches the app when it finishes.

```bash
npm run dist:win        # → dist/Talkobold Setup <version>.exe
```
...
After install (reboot if asked), Talkobold plays into *CABLE Input* and apps
...
wpctl status | grep -i talkobold   # (phase 3) confirm the virtual mic exists
```
